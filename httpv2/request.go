    ...
    */
    ...
    if req.GetHeader("Transfer-Encoding") == "chunked" {
	    ...
	    ChunkedLoop:
            ...
            var nr,nw,totalw int
            var er, ew os.Error
            var towrite int
            length,_ := strconv.Atoi(req.GetHeader("Content-Length"))
        Loop:
            for {
                
                if nr,er = req.Body.Read(buf); nr > 0 && totalw < length {
                    
                    if er == nil || er == os.EOF {
                        if totalw+nr > length {
                            towrite = length - totalw
                        } else
                            towrite = nr
                        }
                        nw,ew = w.Write(buf[0:towrite])
